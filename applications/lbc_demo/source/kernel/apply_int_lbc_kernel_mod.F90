...
module apply_int_lbc_kernel_mod
  ...
  !> Psy layer.
  !>
  type, public, extends(kernel_type) :: apply_int_lbc_kernel_type
    private
     type(arg_type) :: meta_args(2) = (/                         &
         arg_type(GH_FIELD,  GH_INTEGER, GH_WRITE, ANY_SPACE_1), & ! lam_field
         arg_type(GH_FIELD,  GH_INTEGER, GH_READ,  ANY_SPACE_2)  & ! lbc_field
         /)
    integer :: operates_on = CELL_COLUMN
  contains
    procedure, nopass :: apply_int_lbc_kernel_code
  end type apply_int_lbc_kernel_type

  !---------------------------------------------------------------------------
  ! Contained functions/subroutines
  !---------------------------------------------------------------------------
  public :: apply_int_lbc_kernel_code
...
subroutine apply_int_lbc_kernel_code( nlayers,     &
                                      lam_field,   &
                                      lbc_field,   &
                                      undf_lam,    &
                                      map_lam,     &
  ...
  ! Fields
  integer(i_def), intent(inout) :: lam_field( undf_lam )
  integer(i_def), intent(in)    :: lbc_field( undf_lbc )
...
end subroutine apply_int_lbc_kernel_code

end module apply_int_lbc_kernel_mod
