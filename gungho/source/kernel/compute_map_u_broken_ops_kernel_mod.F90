...
!-----------------------------------------------------------------------------
!> @brief Computes the operators for rhs for the mapping of the broken W2
!> wind field from the finite difference W3 lat, W3 lon and Wtheta
!> up computational wind components
!>
module compute_map_u_broken_ops_kernel_mod
  ...
  !> Psy layer.
  !>
  type, public, extends(kernel_type) :: compute_map_u_broken_ops_kernel_type
    private
  ...
  contains
    procedure, nopass :: compute_map_u_broken_ops_code
  end type

!-----------------------------------------------------------------------------
! Contained functions/subroutines
!-----------------------------------------------------------------------------
public :: compute_map_u_broken_ops_code

contains

!> @brief Compute the operators for evaluate the right hand side to map
!!        (lon,lat,r) components of the wind into W2 broken
!! @param[in] cell     Cell number
!! @param[in] nlayers Number of layers
!! @param[in] ncell_3d_1 ncell*ndf
!! @param[in,out] u_lon_op Operator to map u_lon from W3 to W2b
!! @param[in] ncell_3d_2 ncell*ndf
!! @param[in,out] u_lat_op Operator to map u_lat from W3 to W2b
!! @param[in] ncell_3d_3 ncell*ndf
!! @param[in,out] u_up_op Operator to map u_up from WTHETA to W2b
!! @param[in] chi_1 1st coordinate in spherical Wchi
!! @param[in] chi_2 2nd coordinate in spherical Wchi
!! @param[in] chi_3 3rd coordinate in spherical Wchi
!! @param[in] panel_id Field giving the ID for mesh panels
!! @param[in] ndf_w2b Number of degrees of freedom per cell for w2b
!! @param[in] basis_w2b W2b basis functions evaluated at quadrature points
...
subroutine compute_map_u_broken_ops_code(                                &
                                          cell, nlayers,                 &
                                          ncell_3d_1, u_lon_op,          &
                                          ncell_3d_2, u_lat_op,          &
                                          ncell_3d_3, u_up_op,           &
  ...
  real(kind=r_def), intent(in), dimension(3,ndf_w2b, nqp_h,nqp_v) :: basis_w2b
  ...
  real(kind=r_def), dimension(ndf_w2b,ndf_wt,ncell_3d_3), intent(inout) :: u_up_op
  ...
  real(kind=r_def), dimension(3)               :: u_lon_phys_basis, u_lon_basis, &
                                                  u_lat_phys_basis, u_lat_basis, &
                                                  u_up_phys_basis, u_up_basis,   &
                                                  coords, llr
        ...
        do qp1 = 1, nqp_h
          ...
          do df3 = 1, ndf_w3
            do df2 = 1, ndf_w2b
              ...
              if ( geometry == geometry_spherical ) then
                u_lon_basis(:) = 0.0_r_def
                u_lat_basis(:) = 0.0_r_def
                u_lon_basis(1) = basis_w3(1,df3,qp1,qp2)
                u_lat_basis(2) = basis_w3(1,df3,qp1,qp2)
                u_lon_phys_basis = sphere2cart_vector(u_lon_basis,llr)
                u_lat_phys_basis = sphere2cart_vector(u_lat_basis,llr)
                ...
                call log_event('compute_map_u_broken_ops_kernel is not implemented ' //    &
                                'with your geometry',       &
                                LOG_LEVEL_ERROR)
              ...
              integrand = dot_product(matmul(jacobian(:,:,qp1,qp2),&
                                             basis_w2b(:,df2,qp1,qp2)),u_lon_phys_basis)
              u_lon_op(df2,df3,ik) = u_lon_op(df2,df3,ik) &
                                    + wqp_h(qp1)*wqp_v(qp2)*integrand
              integrand = dot_product(matmul(jacobian(:,:,qp1,qp2),&
                                             basis_w2b(:,df2,qp1,qp2)),u_lat_phys_basis)
         ...
         do dft = 1, ndf_wt
           do df2 = 1, ndf_w2b
             ...
             if ( geometry == geometry_spherical ) then
              ...
              u_up_basis(:) = 0.0_r_def
              u_up_basis(3) = basis_wt(1,dft,qp1,qp2)
              u_up_phys_basis   = sphere2cart_vector(u_up_basis,llr)
              ...
              integrand = dot_product(matmul(jacobian(:,:,qp1,qp2),&
                                             basis_w2b(:,df2,qp1,qp2)),u_up_phys_basis)
...
end subroutine compute_map_u_broken_ops_code

end module compute_map_u_broken_ops_kernel_mod
